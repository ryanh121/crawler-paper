...
url = 'https://openreview.net/notes?invitation=ICLR.cc%2F2017%2Fconference%2F-%2Fsubmission'

data_path = '/mnt/d/Research/data/iclr2017conference/'

keylist = [u'replyCount', u'signatures', u'nonreaders', u'forum', u'readers', u'replyto', u'writable',
           u'tmdate', u'number', u'tcdate', u'content', u'tddate', u'writers', u'invitation', u'id', u'ddate', u'tags']
content_keylist = [u'title', u'abstract', u'conflicts',
                   u'authorids', u'keywords', u'authors', u'TL;DR', u'pdf']

# static/images/pdf_icon_blue.svg)].*$', re.DOTALL)
regex_title = re.compile('^.*#(?P<title>[^#]*)\[!\[\]\(\/.*$', re.DOTALL)
regex_abstract = re.compile(
    '^.*Abstract:\*\*(?P<abstract>[^\n]*)\n  \* \*\*(TL|Conflict|Keyword).*$', re.DOTALL)
regex_authorid = re.compile(
    '^.*\*\*Authorids:\*\*(?P<authorid>[^\*\n]*)\n.*$', re.DOTALL)
regex_tldr = re.compile(
    '^.*DR:\*\*(?P<tldr>[^\*\n]*)\n  \* \*\*(Keywords|Conflicts|Authorid).*$', re.DOTALL)

with open(data_path + 'cache_json', 'r') as fin:
...
def dfs_tree(rootid, nodeid, prefix, fout, subsubpath, id2children, id2signature, id2tmdate, id2tcdate, id2content):
    ...
    try:
        ...
        for child in children:
            dfs_tree(rootid, child, prefix + '\t', fout, subsubpath,
                     id2children, id2signature, id2tmdate, id2tcdate, id2content)
    except KeyError:
        return
    ...
    for j in range(0, len(keylist)):
        ...
        if keyj == 'content':
            ...
            for k in range(0, len(content_keylist)):
                ...
                if subkeyk in set(['conflicts', 'authorids', 'keywords', 'authors']):
...
def write_abstract(sub_path, forumcode):
    ...
    authorid = result_authorid.group('authorid').lstrip().rstrip()
...
def write_first_version(sub_path, forumcode):
    ...
    first_version_id = str(json.loads(urllib2.urlopen(jsonurl).read())[
                           u'references'][-1][u'id'])
    suburl = "https://openreview.net/references/pdf?id=" + first_version_id
    try:
        page = urllib2.urlopen(suburl)
    except urllib2.HTTPError:
        suburl = str(json.loads(urllib2.urlopen(jsonurl).read())[u'references'][-1][u'content'][u'pdf'])
        page = urllib2.urlopen(suburl)
    time.sleep(2)
...
def write_review(sub_path, forumcode):
    ...
    for j in range(0, len(subjsonresult)):
        try:
            ...
            if subjsonresult[j][u'signatures'] == [] and subjsonresult[j][u'content'][u'authors'] == [] and subjsonresult[j][u'writers'] == [] and (u'title' not in subjsonresult[j][u'content']):
                ...
                raise ValueError(
                    'empty content ' + subjsonresult[j][u'id'] + ' ' + str(i) + ' ' + forumcode)
    ...
    for child in children:
        ...
        dfs_tree(rootid, child, '', fout, subsubpath, id2children,
                 id2signature, id2tmdate, id2tcdate, id2content)
        fout.close()


for i in range(474, len(json_result['notes'])):
    print 'crawling', i
